...
void SideSensor::updateStatus() // called timer flip
{
	...
	if(ignore_flag_ == false){
		if(rise_fall_flag_R_ == true && timer_R_ >= 3){
			status_R_ = false;
		}
		else if(rise_fall_flag_R_ == false && timer_R_ >= 3){
			status_R_ = true;
		}

		if(rise_fall_flag_L_ == true && timer_L_ >= 3){
			status_L_ = false;
		}
		else if(rise_fall_flag_L_ == false && timer_L_ >= 3){
			status_L_ = true;
		}
	}
